...
- 不得删除 WGC、DXGI、D3D11、Win32 原实现；它们必须仅在 Windows 目标中编译。
...
目标：从 `src/main.cpp` 抽出平台无关的 shader 和绘制逻辑，Windows 行为不变。
...
4. Windows `main.cpp` 只负责窗口、捕获、进程与平台事件，调用共享渲染核心。
...
2. 变更是否严格在当前任务包范围。
3. 是否意外删除 Windows 能力。
4. CMake configure/build 是否无隐式工作目录依赖。
5. `ctest --output-on-failure`。
6. 新增错误路径是否有错误信息、清理和可测试结果。
7. 线程、D-Bus request、PipeWire buffer、fd、GL 资源的所有权是否明确。
8. 手工运行是否与 DeepSeek 报告一致。
...
- Windows 源码和构建定义仍存在，没有为 Linux 迁移破坏上游功能。
