...
> **历史文档说明（2026-07-14）**：本文记录已经完成的早期迁移实施过程，不再是现行产品计划。当前项目只面向本机 Ubuntu 26.04、GNOME 50、Wayland、RTX 3060、单显示器；不维护 Windows，不实现多屏，不继续迁移 Windows 专属非核心功能，也不再以 Portal/独立 renderer 为正式产品路径。现行范围与收尾计划见根目录 `UPSTREAM_1.2.1_UPDATE_ANALYSIS.md` 的第 1.4、4、5、6、7 节。本文后续出现的跨平台、多屏、Portal 和待实施描述仅作历史记录，不构成当前验收要求。
...
| 全局停止热键 | Win32 `RegisterHotKey` | GNOME Shell 扩展 `Main.wm.addKeybinding` + GSettings |
...
- 快捷键归 GNOME Shell 扩展所有，使用 `Main.wm.addKeybinding`，不读取原始键盘事件。
- Qt 设置页沿用现有启用开关和组合键录制，通过本地会话 D-Bus 调用扩展更新 GSettings。
- 默认组合键为 `Ctrl+Alt+B`；触发后立即停止合成器效果，并通知 UI 停止空闲检测，避免自动重启。
- 不引入 JNativeHook、JNI/JNA、X11 XRecord 或管理员权限；不实现通用按键监听和录制捕获热键。

退出条件：GNOME 50 Wayland 下，UI 前台、后台以及原生 Wayland 应用获得焦点时均能停止效果；禁用或修改快捷键后立即生效，扩展重载后配置保持。
...
1. 每个 `REVIEW_FINDING` 必须分别对应一处代码改动和一条可复现证据，不得只报告“全部修复”。
2. 开发中先运行受影响的最小编译/测试；送审前再使用 `/tmp/blackhole-<task>-*` 进行全新完整构建。
