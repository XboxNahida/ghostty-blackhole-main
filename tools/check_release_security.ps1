...
function Resolve-Objdump {
    ...
    foreach ($cachePath in @(
        (Join-Path $root "_build_v124_renderer\CMakeCache.txt"),
        (Join-Path $root "_build_v124_ui\CMakeCache.txt")
    )) {
...
foreach ($executable in $executables) {
    ...
    if ($version.ProductVersion -cne "1.2.4") {
        Add-Failure "$($executable.Name) ProductVersion is [$($version.ProductVersion)]"
    }
    if ($version.FileVersion -cne "1.2.4.0") {
...
foreach ($contract in @(
    @{ Pattern = '\$UiBuildDir\s*=\s*Join-Path\s+\$ProjectRoot\s+"_build_v124_ui"'; Message = "package script does not use the source-local short Qt UI build directory" },
    @{ Pattern = '\$CoreBuildDir\s*=\s*Join-Path\s+\$ProjectRoot\s+"_build_v124_renderer"'; Message = "package script does not use the source-local short renderer build directory" },
...
foreach ($requiredBuildPath in @("_build_v124_renderer", "_build_v124_ui")) {
...
$buildArtifacts = @(
    @{ Path = (Join-Path $root "_build_v124_renderer\blackhole.exe"); HashField = "BuildRendererSHA256" },
    @{ Path = (Join-Path $root "_build_v124_ui\appBlakholeUI.exe"); HashField = "BuildUiSHA256" }
)
...
Write-Output "RELEASE_SECURITY_OK version=1.2.4"
