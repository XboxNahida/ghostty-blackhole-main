...
if ([string]::IsNullOrWhiteSpace($RendererPath)) {
    $RendererPath = Join-Path $root "_build_v124_renderer\blackhole.exe"
}
if ([string]::IsNullOrWhiteSpace($UiPath)) {
    $UiPath = Join-Path $root "_build_v124_ui\appBlakholeUI.exe"
}
...
foreach ($builtFile in $builtFiles) {
    ...
    if ($version.ProductVersion -cne "1.2.4") {
        Fail-IdentityCheck "$(Split-Path -Leaf $path) ProductVersion is $($version.ProductVersion), expected 1.2.4"
    }
    if ($version.FileVersion -cne "1.2.4.0") {
        Fail-IdentityCheck "$(Split-Path -Leaf $path) FileVersion is $($version.FileVersion), expected 1.2.4.0"
    }
...
"RELEASE_IDENTITY_OK version=1.2.4"
