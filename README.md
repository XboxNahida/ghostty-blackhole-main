...
- Qt 6/QML 配置界面与实时预览；预览同步洞大小、移动速度、旋转速度和固定/往返缩放。
- GNOME 效果每次启动可随机选择黑洞出生位置，也可在高级设置中固定到四角。
...
本轮只保留了通过自动测试和 GNOME 真屏验收的优化。正式后测在临时停用动态壁纸、将空闲阈值从 30 秒提高到 3600 秒后进行，避免视频背景和自动空闲触发污染样本；每个状态预热 5 秒、采样 30 秒，重复 3 轮取中位数。

### 已证明的收益

| 指标 | 优化前 | 优化后 | 变化 |
|---|---:|---:|---:|
| 最小化 UI CPU | 0.30% | 0.033% | 约 -89% |
| Linux idle 固定轮询 | 1 次/秒 | 0 | -100% |
| MPRIS `ListNames` 固定轮询 | 1 次/2 秒 | 0 | -100% |
| UI 后台 cgroup 内存 | 69.5 MiB | 60.2 MiB | -9.3 MiB / -13.4% |
| UI cgroup 内存峰值 | 81.1 MiB | 74.9 MiB | -6.2 MiB / -7.6% |
| UI 线程/任务 | 11 | 10 | 约 -9% |
| 扩展启用时的 shader 配置构建 | 2 份 | 1 份 | -50% |
| 配置未变时的后续 `Start` 构建 | 2 份 | 0 | -100% |
| `ReloadConfig` 配置读取/拼接 | 2 份 | 1 份 | -50% |

Linux 空闲检测和 MPRIS 现在都由 D-Bus 事件驱动。UI 在最小化冷启动时不再创建预览 FBO，大预览仅在打开弹窗时存在，关闭后释放图形资源。扩展端对配置指纹和组装后的 shader source 进行会话级缓存。

### 未达成的指标

- `pidstat` RSS 口径的 UI 内存从 203.4 MiB 变为 221.8 MiB。RSS 与 cgroup charged memory 口径不同，因此不声称“UI RSS 低于 120 MiB”已达成。
- 校正后三轮采样中，GNOME Shell 停止效果时的 CPU 中位数为 13.329%，稳定运行时为 39.919%；每轮运行减停止的配对增量中位数为 27.733 个百分点。当前长时间 GNOME Shell 会话的基础负载远高于立项时，不能把该数值与旧的 1.80 个百分点当成纯代码前后对比；但可以确认本轮没有保留稳态 GNOME Shell CPU 优化。
- 无动态壁纸时，效果运行相对停止约增加 30.24 个 GPU SM 百分点、21.2 MiB VRAM 和 7.11 W；本轮不声称 GPU 渲染成本已优化。

更完整的原始数据口径、回退原因和每轮记录见 [CPU 与资源优化计划](docs/superpowers/plans/2026-07-15-cpu-resource-optimization.md)。

## 已知小问题

- 每次重新登录 GNOME Shell 会话后，本会话第一次启动黑洞时可能在正常画面出现前短暂全屏黑闪一次。同一会话中的后续启动正常，停止效果后桌面也能正常恢复。
- 已尝试 shader 透明预热等修复，但未能在不引入视觉回归或额外复杂度的前提下消除黑闪，相关实验代码已回退。当前版本将此作为不阻断使用的已知限制保留。
...
除上述会话首次启动短暂黑闪外，当前本机核心链路没有已知阻塞问题。真实 MPRIS 播放器、锁屏/解锁和睡眠/唤醒测试
