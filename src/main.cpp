...
static bool buildFragmentShader(std::string& out, FILE* debugLog) {
    ...
    // ---- Runtime visual controls ----
    {
        ...
        const std::string newNear =
            ...
            "            float diskBandDistance = abs(r - diskMid) / max(diskSpan, 0.003);\n"
            "            float dimensionCollapse = smoothstep(1.20 + boundaryDither, 0.08, diskBandDistance) * smoothstep(diskOuterScreen + diskSpan * 0.85, diskInnerScreen * 0.48, r) * swallowPhase;\n"
            "            float softCollapse = smoothstep(0.08, 0.92, dimensionCollapse);\n"
            "            float adaptiveCollapse = softCollapse * (0.58 + 0.30 * ringBirth);\n"
            "            float normalizedDiskDistance = clamp((r - diskInnerScreen) / max(diskSpan, 0.003), 0.0, 1.0);\n"
            ...
            "            vec2  adaptiveDiskP = p + lensVector * swallowLensField * (1.0 + 1.35 * adaptiveCollapse) + tangentDir * tangentialStretch + radialDir * radialCompression;\n"
        ...
        if (p != std::string::npos) {
            body.replace(p, finalColor.length(),
                ...
                "        float uiSuppression = clamp(realAccretionMask * (0.82 + 0.14 * swallowPhase) + eventHorizonMask, 0.0, 1.0);\n"
                "        float bgLuma = dot(bg, vec3(0.2126, 0.7152, 0.0722));\n"
                "        vec3 thermalColor = blackbody(max(L.temp, 1500.0));\n"
                "        vec3 deUiBg = mix(bg, vec3(bgLuma) * 0.16 + thermalColor * bgLuma * 0.10, realAccretionMask);\n"
                "        vec3 programmaticAccretion = diskLight * (1.0 + 0.85 * swallowPhase) + thermalColor * realAccretionMask * (0.18 + 0.30 * swallowPhase);\n"
                "        vec3 swallowedColor = deUiBg * trans * (1.0 - realAccretionMask * 0.92) + programmaticAccretion;\n"
