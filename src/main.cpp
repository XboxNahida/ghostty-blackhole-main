...
static bool buildFragmentShader(std::string& out, FILE* debugLog) {
    ...
    // ---- Runtime visual controls ----
    {
        ...
        const std::string newNear =
            ...
            "            float dimensionCollapse = smoothstep(0.56, 0.12, r) * smoothstep(0.030, 0.18, r) * swallowPhase;\n"
            "            float collapseToDisk = smoothstep(0.18, 0.92, dimensionCollapse);\n"
            ...
            "            float eventHorizonMask = smoothstep(0.14, 0.038, r) * swallowPhase;\n"
        ...
        if (p != std::string::npos) {
            body.replace(p, finalColor.length(),
                ...
                "        float dimensionCollapse = smoothstep(0.56, 0.12, diskR) * smoothstep(0.030, 0.18, diskR) * swallowPhase;\n"
                "        float collapseToDisk = smoothstep(0.18, 0.92, dimensionCollapse);\n"
                ...
                "        float stretchedMatter = smoothstep(0.08, 0.88, collapseToDisk) * (0.55 + 0.45 * abs(sin(diskPhase * 5.0 + diskR * 11.0)));\n"
                "        vec3 accretionMatter = vec3(0.018, 0.016, 0.025) + vec3(0.42, 0.25, 0.10) * ringGlow + vec3(0.20, 0.16, 0.12) * stretchedMatter;\n"
                "        col = mix(col, col * mix(0.30, 0.62, outerLensBlend), swallowPhase * outerLensBlend * 0.16);\n"
                "        col = mix(col, col * mix(0.10, 0.38, stretchedMatter) + accretionMatter, collapseToDisk);\n"
                "        float eventHorizonMask = smoothstep(0.14, 0.036, diskR) * swallowPhase;\n"
