    ...
    // ---- Runtime visual controls ----
    {
        ...
        if (p != std::string::npos) {
            body.replace(p, finalColor.length(),
                ...
                "        float tidalUiErase = clamp(max(nearFieldUiCutoff * (0.80 + 0.18 * swallowPhase), wideTidalErase * 0.86), 0.0, 1.0);\n"
                ...
                "        float uiDebrisSuppression = clamp(max(tidalUiErase, wideTidalErase * (0.72 + 0.18 * swallowPhase)), 0.0, 1.0);\n"
