...
static bool buildFragmentShader(std::string& out, FILE* debugLog) {
    ...
    // ---- Runtime visual controls ----
    {
        ...
        const std::string newNear =
            ...
            "            float lensAmplification = 1.0 + swallowPhase * (2.2 + 2.8 * smoothstep(0.08, 0.95, originalWindow));\n"
            "            vec2  baseLensP = p + (sp - p) * originalWindow * baseDistortion;\n"
            "            vec2  lensP = p + (sp - p) * originalWindow * baseDistortion * lensAmplification;\n"
            "            float r = max(length(lensP), 0.0005);\n"
            "            float theta = atan(lensP.y, lensP.x);\n"
            "            vec2  radialDir = lensP / r;\n"
            "            vec2  tangentDir = vec2(-radialDir.y, radialDir.x);\n"
            "            float outerLensBlend = smoothstep(0.03, 0.72, originalWindow) * swallowPhase;\n"
            "            float boundaryDither = 0.018 * sin(theta * 5.0 + iTime * 0.36) + 0.010 * sin(theta * 11.0 - iTime * 0.21);\n"
            "            float dimensionCollapse = smoothstep(0.58 + boundaryDither, 0.11 + boundaryDither * 0.35, r) * smoothstep(0.026, 0.15, r) * swallowPhase;\n"
            "            float softCollapse = smoothstep(0.10, 0.96, dimensionCollapse);\n"
            "            float collapseToDisk = softCollapse * (0.68 + 0.24 * ringBirth);\n"
            "            float ringWidth = mix(0.010, 0.060, ringBirth * swallowPhase);\n"
            "            float ringRadius = mix(0.18, 0.34, swallowPhase);\n"
            "            float diskSpin = clamp((0.11 + 0.17 * swallowPhase) / (0.12 + r), 0.0, 2.2);\n"
            "            float diskPhase = theta + iTime * diskSpin;\n"
            "            float ringGlow = exp(-pow((r - ringRadius) / max(ringWidth, 0.004), 2.0)) * ringBirth * swallowPhase;\n"
            "            float luminosityBoost = 1.0 + ringGlow * 1.10 + softCollapse * 0.20;\n"
            "            float tangentialStretch = collapseToDisk * (0.18 + 0.62 / (0.18 + r));\n"
            "            float radialCompression = collapseToDisk * (0.72 + 0.22 * sin(diskPhase * 2.0));\n"
            "            vec2  diskLineP = lensP + tangentDir * tangentialStretch + radialDir * radialCompression;\n"
            "            float spiralAngle = diskSpin * (0.18 + 0.62 * collapseToDisk) + ringGlow * 0.18;\n"
            "            mat2  spiralRot = mat2(cos(spiralAngle), -sin(spiralAngle), sin(spiralAngle), cos(spiralAngle));\n"
            "            vec2  collapseP = spiralRot * diskLineP;\n"
            "            float eventHorizonMask = smoothstep(0.090, 0.026, r) * swallowPhase;\n"
            "            vec2  finalP = mix(mix(baseLensP, lensP, outerLensBlend), collapseP, collapseToDisk);\n"
        ...
        if (p != std::string::npos) {
            body.replace(p, finalColor.length(),
                ...
                "        float boundaryDither = 0.018 * sin(diskTheta * 5.0 + iTime * 0.36) + 0.010 * sin(diskTheta * 11.0 - iTime * 0.21);\n"
                "        float dimensionCollapse = smoothstep(0.58 + boundaryDither, 0.11 + boundaryDither * 0.35, diskR) * smoothstep(0.026, 0.15, diskR) * swallowPhase;\n"
                "        float softCollapse = smoothstep(0.10, 0.96, dimensionCollapse);\n"
                "        float collapseToDisk = softCollapse * (0.68 + 0.24 * ringBirth);\n"
                "        float outerLensBlend = smoothstep(0.26, 0.88, diskR) * (1.0 - collapseToDisk);\n"
                "        float ringWidth = mix(0.010, 0.060, ringBirth * swallowPhase);\n"
                "        float ringRadius = mix(0.18, 0.34, swallowPhase);\n"
                "        float diskSpin = clamp((0.11 + 0.17 * swallowPhase) / (0.12 + diskR), 0.0, 2.2);\n"
                "        float diskPhase = diskTheta + iTime * diskSpin;\n"
                "        float ringGlow = exp(-pow((diskR - ringRadius) / max(ringWidth, 0.004), 2.0)) * ringBirth * swallowPhase;\n"
                "        float luminosityBoost = 1.0 + ringGlow * 1.25 + softCollapse * 0.22;\n"
                "        float stretchedMatter = smoothstep(0.08, 0.88, softCollapse) * (0.55 + 0.45 * abs(sin(diskPhase * 5.0 + diskR * 11.0)));\n"
                "        vec3 accretionMatter = vec3(0.034, 0.030, 0.043) + vec3(0.64, 0.38, 0.14) * ringGlow + vec3(0.30, 0.23, 0.17) * stretchedMatter;\n"
                "        col = mix(col, col * mix(0.30, 0.62, outerLensBlend), swallowPhase * outerLensBlend * 0.16);\n"
                "        vec3 collapseColor = col * mix(0.54, 0.92, stretchedMatter) * luminosityBoost + accretionMatter;\n"
                "        col = mix(col, collapseColor, collapseToDisk * (0.56 + 0.30 * ringGlow));\n"
                "        float eventHorizonMask = smoothstep(0.090, 0.024, diskR) * swallowPhase;\n"
