...
$UiBuildDir = Join-Path $ProjectRoot "_build_v124_ui"
$CoreBuildDir = Join-Path $ProjectRoot "_build_v124_renderer"
...
if (-not (Test-Path -LiteralPath $RendererSource)) {
    throw "Missing _build_v124_renderer\blackhole.exe. Run: cmake --build _build_v124_renderer --config Release"
}

if (-not (Test-Path -LiteralPath $UiSource)) {
    throw "Missing Qt UI build output. Run: cmake --build _build_v124_ui --config Release"
}
...
if ((Get-FileHash -Algorithm SHA256 -LiteralPath $RendererSource).Hash.ToLowerInvariant() -cne $rendererBuildHash) {
    throw "Packaging modified _build_v124_renderer\blackhole.exe"
}
